...
#        self.initialAxis = copy_axis( axis )
        ...
        Trange = None #getattr(customizeUVCDAT,"timeRange",None)
        Zrange = None #getattr(customizeUVCDAT,"levelRange",None)
        Yrange = None #getattr(customizeUVCDAT,"latitudeRange",None)
        Xrange = None #getattr(customizeUVCDAT,"longitudeRange",None)
        if self.isModuloed:
    ...
    def checkBounds( self, bounds, dataset ):
        ...
        if ( bounds[0] < values[0] ) or ( bounds[1] > values[-1] ):
            ...
            if dataset == None: QtGui.QMessageBox.warning( self, "UVCDAT Warning", "Selected bounds are out of range for the transient variable. Please reaccess this variable from the dataset.")
            else:               QtGui.QMessageBox.warning( self, "UVCDAT Warning", "Selected bounds are out of range for the dataset.")
        return bounds
#            self.resetValues()
...
class QAxisComboWidget(QtGui.QComboBox):
    ...
    def __init__(self, parent=None):
        ...
        # Connect Signals
        self.currentIndexChanged.connect(self.valueChangedEvent)
        self.editTextChanged.connect(self.valueChangedEvent)
